  ...
  const App = {
    ...
    HB_SYS() {
      return [
        ...
        '',
        '【播放与控制】',
        '9. 说“放/播放” → play_music；说“下一首/暂停/继续/上一首” → control；音量、进度、音质（如无损/母带）、播放模式、收藏当前歌曲 → 用对应工具。要能响应用户的自然说法，例如“放一首适合下雨天的歌 / 音量 30% / 下一首 / 换成无损”。',
        '10.【能力对齐】你在界面上能做的 AI 都能做：播放控制、搜索、读取音乐库、分享链接（share_current / share_song / share_playlist）、收藏歌单、新建歌单并加歌、最近播放、切换主题、下载歌曲、打开歌曲详情、跳转页面；听歌识曲由用户点击底部「听歌识曲」按钮触发，识别成功后你负责介绍并推荐同类歌。用户提出这类需求时直接调用对应工具，绝不说“没有这个功能”。',
