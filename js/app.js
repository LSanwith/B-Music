...
(function () {
  ...
  const App = {
    ...
    HB_SYS() {
        ...
        '5b) 【搜索次数】优先一次搜索到位；若结果不足 4 首，可换更宽的关键词再搜（本轮最多 3 次）；正文只列 4 首。' +
        '5d) 【严禁幻觉】正文里出现的歌名/歌手必须与 search_music 结果的 songs 字段【逐字一致】（直接复制），不得替换成你记忆里的其它歌、不得翻译或改写；正文只允许出现这些歌，一首都不许多写。' +
        '4b) 【输出格式·必须严格遵守】当你要推荐歌曲时，回复正文【只能】是下面这种格式，不要任何其它文字（不要开场白、不要总结、不要“需要我…”）：\n' +
        '歌名 —— 歌手 —— 一句不超过 20 字的推荐语\n' +
        '（正好 4 行，每行一首，行与行之间不要空行；歌名与歌手必须逐字来自 search_music 的返回结果）' +
